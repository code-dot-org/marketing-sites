...
interface SitemapEntry {
  slug: string;
  noIndex?: boolean;
  // LEGACY-ENV-COMPAT: the old environment stores noindex on a linked entry.
...
async function getSitemapEntries(
  ...
  try {
    ...
    // LEGACY-ENV-COMPAT: the old environment has no `noIndex` field on the
    // experience (422 UnknownField); noindex lives on the linked seoMetadata
    // entry instead. Remove this fallback when the production Contentful
    // environment serves the new content model.
...
export async function GET(request: Request) {
  ...
  for (const entry of experienceEntries) {
    ...
    // Skip entries marked as noindex (LEGACY-ENV-COMPAT: old environments
    // store the flag on the linked seoMetadata entry)
    if (
